...
export default function POsPage() {
  ...
  const renderContextMenu = (po: PO) => {
    ...
    return (
      <div ref={menuRef} className="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-20 py-1 overflow-hidden">
        <button onClick={() => openDetailModal(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
          <Eye size={15} className="text-slate-400" /> Ver detalles
        </button>
        <button onClick={() => { generatePDF(po); setOpenMenuId(null); }} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
          <Download size={15} className="text-slate-400" /> Descargar PDF
        </button>
        {po.status === "draft" && (
          <>
            ...
            <button onClick={() => handleEditDraft(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
              <Edit size={15} className="text-slate-400" /> Editar borrador
            </button>
        ...
        {po.status === "approved" && (
          <>
            ...
            <button onClick={() => handleCreateInvoice(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
              <Receipt size={15} className="text-slate-400" /> Crear factura
            </button>
            <button onClick={() => handleModifyPO(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
              <FileEdit size={15} className="text-slate-400" /> Modificar PO
            </button>
            <button onClick={() => handleClosePO(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
              <Lock size={15} className="text-slate-400" /> Cerrar PO
            </button>
            {po.invoicedAmount === 0 && (
        ...
        {po.status === "closed" && (
          <>
            ...
            <button onClick={() => handleReopenPO(po)} className="w-full px-4 py-2.5 text-left text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3">
              <Unlock size={15} className="text-slate-400" /> Reabrir PO
            </button>
          </>
        )}
      </div>
    ...
    <div className={`min-h-screen bg-white ${inter.className}`}>
      <div className="mt-16 border-b border-slate-200">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <Link href={`/project/${id}/accounting`} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 mb-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{pos.length} {pos.length === 1 ? "orden" : "órdenes"}</p>
              </div>
            </div>
            <Link href={`/project/${id}/accounting/pos/new`} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-6 py-6">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Buscar por número, proveedor..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 bg-white text-sm" />
          </div>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 bg-white text-sm">
        ...
        {filteredPOs.length === 0 ? (
          <div className="border-2 border-dashed border-slate-200 rounded-xl p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-1">{searchTerm || statusFilter !== "all" ? "No se encontraron POs" : "No hay órdenes de compra"}</h3>
            <p className="text-slate-500 text-sm mb-4">{searchTerm || statusFilter !== "all" ? "Intenta ajustar los filtros" : "Comienza creando tu primera orden de compra"}</p>
            {!searchTerm && statusFilter === "all" && (
              <Link href={`/project/${id}/accounting/pos/new`} className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800">
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-right px-6 py-3 text-xs font-medium text-slate-500 uppercase">Base</th>
                ...
                {filteredPOs.map((po) => (
                  <tr key={po.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <p className="text-sm font-semibold text-slate-900">{formatCurrency(po.baseAmount || po.totalAmount)} €</p>
                      {po.status === "approved" && po.invoicedAmount > 0 && <p className="text-xs text-emerald-600">Fact: {formatCurrency(po.invoicedAmount)} €</p>}
                    </td>
                    <td className="px-6 py-4">{getStatusBadge(po.status)}</td>
                    <td className="px-6 py-4">
                      <div className="relative">
                        <button onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === po.id ? null : po.id); }} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => generatePDF(selectedPO)} className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors" title="Descargar PDF">
                  <Download size={18} />
                </button>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              {(selectedPO.status === "approved" || selectedPO.status === "closed") && (
                <div className="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-lg">
                  <p className="text-xs font-medium text-slate-700 uppercase mb-3">Control presupuestario</p>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Comprometido</p>
                      <p className="text-sm font-semibold text-amber-600">{formatCurrency(selectedPO.committedAmount)} €</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Facturado</p>
                      <p className="text-sm font-semibold text-emerald-600">{formatCurrency(selectedPO.invoicedAmount)} €</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Pendiente</p>
                      <p className="text-sm font-semibold text-blue-600">{formatCurrency(selectedPO.remainingAmount)} €</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="mb-6">
                <p className="text-xs font-medium text-slate-700 uppercase mb-3">Items ({selectedPO.items?.length || 0})</p>
                {loadingInvoices ? (
                  <div className="text-center py-4 text-slate-500 text-sm">Cargando información de facturación...</div>
                ) : (
                  <div className="space-y-3">
                    {itemsWithInvoiced.map((item, index) => {
                      ...
                      return (
                        <div key={item.id || index} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <p className="text-sm font-semibold text-slate-900">{formatCurrency(itemTotal)} €</p>
                          </div>
                          {(selectedPO.status === "approved" || selectedPO.status === "closed") && (
                            <div className="mt-3 pt-3 border-t border-slate-200">
                              ...
                              <div className="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                                <div className={`h-full transition-all ${isOverInvoiced ? "bg-red-500" : percentInvoiced > 90 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${Math.min(percentInvoiced, 100)}%` }} />
                              </div>
                              <div className="flex items-center justify-between text-xs mt-2">
                                <span className="text-slate-500">Pendiente</span>
                                <span className={`font-medium ${item.pendingAmount < 0 ? "text-red-600" : "text-slate-700"}`}>{formatCurrency(item.pendingAmount)} €</span>
                              </div>
                            </div>
                          )}
                        </div>
              ...
              <div className="mb-6">
                <p className="text-xs font-medium text-slate-700 uppercase mb-3">Facturas vinculadas</p>
                {loadingInvoices ? (
                  <div className="p-4 bg-slate-50 rounded-lg text-center"><p className="text-sm text-slate-500">Cargando...</p></div>
                ) : linkedInvoices.length === 0 ? (
                  <div className="p-4 bg-slate-50 rounded-lg border border-dashed border-slate-300 text-center"><p className="text-sm text-slate-500">No hay facturas vinculadas</p></div>
                ) : (
              ...
              {selectedPO.modificationHistory && selectedPO.modificationHistory.length > 0 && (
                <div className="mb-6">
                  <p className="text-xs font-medium text-slate-700 uppercase mb-3 flex items-center gap-2"><History size={14} /> Historial de modificaciones</p>
                  <div className="space-y-2">
                    {selectedPO.modificationHistory.map((mod, index) => (
                      <div key={index} className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="text-sm font-medium text-purple-900">V{String(mod.previousVersion).padStart(2, "0")} → V{String(mod.previousVersion + 1).padStart(2, "0")}</p>
              ...
              {selectedPO.status === "cancelled" && selectedPO.cancellationReason && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-xs font-medium text-red-800 uppercase mb-2">Motivo de anulación</p>
            ...
            {(selectedPO.status === "draft" || selectedPO.status === "approved" || selectedPO.status === "closed") && (
              <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-2">
                {selectedPO.status === "draft" && (
                  <>
                    <button onClick={() => handleDeleteDraft(selectedPO)} className="px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors">Eliminar</button>
                    <button onClick={() => handleEditDraft(selectedPO)} className="px-4 py-2 text-sm bg-slate-900 text-white hover:bg-slate-800 rounded-lg transition-colors">Editar borrador</button>
                  </>
                )}
                {selectedPO.status === "approved" && (
                  <>
                    {selectedPO.invoicedAmount === 0 && <button onClick={() => handleCancelPO(selectedPO)} className="px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors">Anular</button>}
                    <button onClick={() => handleModifyPO(selectedPO)} className="px-4 py-2 text-sm border border-slate-200 text-slate-700 hover:bg-white rounded-lg transition-colors">Modificar</button>
                    <button onClick={() => handleCreateInvoice(selectedPO)} className="px-4 py-2 text-sm bg-slate-900 text-white hover:bg-slate-800 rounded-lg transition-colors">Crear factura</button>
                  </>
                )}
                {selectedPO.status === "closed" && <button onClick={() => handleReopenPO(selectedPO)} className="px-4 py-2 text-sm bg-slate-900 text-white hover:bg-slate-800 rounded-lg transition-colors">Reabrir PO</button>}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <div className="mb-6">
                ...
                <textarea value={cancellationReason} onChange={(e) => setCancellationReason(e.target.value)} placeholder="Explica por qué se anula esta PO..." rows={4} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 resize-none bg-white text-sm" />
              ...
              <div className="flex gap-3">
                <button onClick={() => { setShowCancelModal(false); setSelectedPO(null); setCancellationReason(""); }} className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 text-sm font-medium transition-colors">Cancelar</button>
                <button onClick={confirmCancelPO} disabled={processing || !cancellationReason.trim()} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50">{processing ? "Anulando..." : "Confirmar anulación"}</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <textarea value={modificationReason} onChange={(e) => setModificationReason(e.target.value)} placeholder="Explica por qué se modifica esta PO..." rows={4} className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-500 resize-none bg-white text-sm" />
              </div>
              <div className="flex gap-3">
                <button onClick={() => { setShowModifyModal(false); setSelectedPO(null); setModificationReason(""); }} className="flex-1 px-4 py-2.5 border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 text-sm font-medium transition-colors">Cancelar</button>
                <button onClick={confirmModifyPO} disabled={processing || !modificationReason.trim()} className="flex-1 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50">{processing ? "Modificando..." : "Modificar"}</button>
              </div>
            </div>
          </div>
        </div>
