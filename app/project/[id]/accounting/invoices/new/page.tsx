...
export default function NewInvoicePage() {
  ...
  const getCompletionPercentage = () => { let c = 0; if (uploadedFile) c++; if (formData.invoiceType === "with-po" ? selectedPO : formData.supplier || linkedDocumentId) c++; if (formData.description.trim()) c++; if (formData.dueDate) c++; if (items.some((i) => i.description.trim() && i.subAccountId && i.quantity > 0 && i.unitPrice > 0)) c++; return Math.round((c / 5) * 100); };
  ...
  return (
    <div className={`min-h-screen bg-white ${inter.className}`}>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-6">
          <div className="mb-4">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 text-xs font-medium">
              ...
              <Link href={`/project/${id}/accounting/invoices`} className="hover:text-slate-900 transition-colors">Facturas</Link>
              <span className="text-slate-300">·</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {(Object.entries(DOCUMENT_TYPES) as [DocumentType, typeof DOCUMENT_TYPES.invoice][]).map(([key, config]) => {
                    ...
                    return (<button key={key} onClick={() => { setDocumentType(key); setLinkedDocumentId(""); }} className={`p-4 rounded-xl border-2 transition-all text-left ${isSelected ? `${config.borderColor} ${config.bgColor}` : "border-slate-200 hover:border-slate-300"}`}>
                      <div className="flex items-center justify-between mb-2"><Icon size={20} className={isSelected ? config.textColor : "text-slate-400"} />{isSelected && <CheckCircle2 size={16} className={config.textColor} />}</div>
                      <p className="font-semibold text-slate-900 text-sm">{config.label}</p><p className="text-xs text-slate-500 mt-0.5">{config.code}</p>
                    </button>);
                  })}
                </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button onClick={() => { setFormData({ ...formData, invoiceType: "with-po", supplier: "", supplierName: "" }); setItems([]); setSelectedPO(null); }} className={`p-5 rounded-xl border-2 transition-all text-left ${formData.invoiceType === "with-po" ? "border-slate-900 bg-slate-50" : "border-slate-200 hover:border-slate-300"}`}>
                    <div className="flex items-center justify-between mb-3"><div className="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center"><FileText size={20} className={formData.invoiceType === "with-po" ? "text-indigo-600" : "text-indigo-400"} /></div>{formData.invoiceType === "with-po" && <CheckCircle2 size={20} className="text-slate-900" />}</div>
                  ...
                  <button onClick={() => { setFormData({ ...formData, invoiceType: "without-po", supplier: "", supplierName: "" }); setItems([]); setSelectedPO(null); }} className={`p-5 rounded-xl border-2 transition-all text-left ${formData.invoiceType === "without-po" ? "border-slate-900 bg-slate-50" : "border-slate-200 hover:border-slate-300"}`}>
                    <div className="flex items-center justify-between mb-3"><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><Receipt size={20} className={formData.invoiceType === "without-po" ? "text-emerald-600" : "text-emerald-400"} /></div>{formData.invoiceType === "without-po" && <CheckCircle2 size={20} className="text-slate-900" />}</div>
                    <h3 className="font-semibold text-slate-900 mb-1">Sin PO</h3><p className="text-sm text-slate-500">Documento independiente</p>
                  </button>
                </div>
              </div>
            </div>}
            ...
            {formData.invoiceType === "with-po" && !replaceMode && <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              ...
              <div className="p-6">
                <button onClick={() => setShowPOModal(true)} onBlur={() => handleBlur("po")} className={`w-full px-4 py-3 border ${hasError("po") ? "border-red-300 bg-red-50" : selectedPO ? "border-emerald-300 bg-emerald-50" : "border-slate-200"} rounded-xl hover:border-slate-300 transition-colors text-left flex items-center justify-between`}>
                  {selectedPO ? <div className="flex items-center gap-3"><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><CheckCircle2 size={18} className="text-emerald-600" /></div><div><p className="font-semibold text-slate-900">PO-{selectedPO.number}</p><p className="text-sm text-slate-500">{selectedPO.supplier}</p></div></div> : <span className="text-slate-400">Seleccionar PO...</span>}
                  <Search size={16} className="text-slate-400" />
                </button>
                {hasError("po") && <p className="text-xs text-red-600 mt-1.5 flex items-center gap-1"><AlertCircle size={12} />{errors.po}</p>}
                {selectedPO && <div className={`mt-4 p-4 rounded-xl border ${poStats.isOverInvoiced ? "bg-red-50 border-red-200" : "bg-slate-50 border-slate-200"}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-8 h-8 ${poStats.isOverInvoiced ? "bg-red-100" : "bg-slate-200"} rounded-lg flex items-center justify-center flex-shrink-0`}>{poStats.isOverInvoiced ? <AlertTriangle size={16} className="text-red-600" /> : <Info size={16} className="text-slate-500" />}</div>
                    <div className="flex-1"><p className={`text-sm font-semibold mb-2 ${poStats.isOverInvoiced ? "text-red-800" : "text-slate-700"}`}>{poStats.isOverInvoiced ? "Excede el total de la PO" : "Estado de facturación"}</p>
                      <div className="grid grid-cols-2 gap-4 text-sm"><div><p className="text-slate-500">Total PO</p><p className="font-semibold text-slate-900">{formatCurrency(poStats.totalAmount)} €</p></div><div><p className="text-slate-500">Facturado</p><p className={`font-semibold ${poStats.isOverInvoiced ? "text-red-600" : "text-emerald-600"}`}>{formatCurrency(poStats.invoicedAmount)} €</p></div></div>
                      <div className="mt-3 w-full bg-slate-200 rounded-full h-2 overflow-hidden"><div className={`h-full ${poStats.percentageInvoiced > 100 ? "bg-red-500" : poStats.percentageInvoiced > 90 ? "bg-amber-500" : "bg-emerald-500"}`} style={{ width: `${Math.min(poStats.percentageInvoiced, 100)}%` }} /></div>
                    </div>
                  </div>
                </div>}
              </div>
            ...
            {formData.invoiceType === "without-po" && !linkedDocumentId && !replaceMode && <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              ...
              <div className="p-6">
                <button onClick={() => setShowSupplierModal(true)} onBlur={() => handleBlur("supplier")} className={`w-full px-4 py-3 border ${hasError("supplier") ? "border-red-300 bg-red-50" : formData.supplier ? "border-emerald-300 bg-emerald-50" : "border-slate-200"} rounded-xl hover:border-slate-300 transition-colors text-left flex items-center justify-between`}>
                  {formData.supplierName ? <div className="flex items-center gap-3"><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><CheckCircle2 size={18} className="text-emerald-600" /></div><span className="font-medium text-slate-900">{formData.supplierName}</span></div> : <span className="text-slate-400">Seleccionar proveedor...</span>}
                  <Search size={16} className="text-slate-400" />
                </button>
              ...
              <div className="p-6">
                <div onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }} className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${isDragging ? "border-indigo-400 bg-indigo-50" : uploadedFile ? "border-emerald-300 bg-emerald-50" : hasError("file") ? "border-red-300 bg-red-50" : "border-slate-200 hover:border-slate-300"}`}>
                  {uploadedFile ? <div className="flex items-center justify-between bg-white border border-slate-200 rounded-xl p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><CheckCircle2 size={18} className="text-emerald-600" /></div><div className="text-left"><p className="text-sm font-medium text-slate-900">{uploadedFile.name}</p><p className="text-xs text-slate-500">{(uploadedFile.size / 1024).toFixed(0)} KB</p></div></div><button onClick={() => setUploadedFile(null)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><X size={16} /></button></div>
                    : <label className="cursor-pointer block"><div className={`w-12 h-12 ${hasError("file") ? "bg-red-100" : "bg-slate-100"} rounded-xl flex items-center justify-center mx-auto mb-3`}><Upload size={20} className={hasError("file") ? "text-red-400" : "text-slate-400"} /></div><p className={`text-sm font-medium mb-1 ${hasError("file") ? "text-red-700" : "text-slate-700"}`}>Arrastra o haz clic para subir</p><p className="text-xs text-slate-500">PDF, JPG, PNG (máx. 10MB)</p><input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileUpload(f); }} className="hidden" /></label>}
                </div>
              ...
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <div className="relative">
                    <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} onBlur={() => handleBlur("description")} placeholder={`Concepto de la ${currentDocType.label.toLowerCase()}...`} rows={2} className={`w-full px-4 py-3 border ${hasError("description") ? "border-red-300 bg-red-50" : formData.description.trim() ? "border-emerald-300 bg-emerald-50" : "border-slate-200"} rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 resize-none text-sm pr-10`} />
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2"><span className="flex items-center gap-2"><Calendar size={14} />Fecha de vencimiento *</span></label>
                    <div className="relative">
                      <input type="date" value={formData.dueDate} onChange={(e) => setFormData({ ...formData, dueDate: e.target.value })} onBlur={() => handleBlur("dueDate")} className={`w-full px-4 py-3 border ${hasError("dueDate") ? "border-red-300 bg-red-50" : formData.dueDate ? "border-emerald-300 bg-emerald-50" : "border-slate-200"} rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 text-sm`} />
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Notas opcionales..." className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 text-sm" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-3"><h2 className="font-semibold text-slate-900">Items</h2><span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${items.length > 0 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{items.length}</span></div>
                <div className="flex items-center gap-2">
                  {formData.invoiceType === "with-po" && selectedPO && <button onClick={() => setShowPOItemsModal(true)} className="flex items-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-medium"><Plus size={14} />De PO</button>}
              ...
              <div className="p-6">
                {items.length === 0 ? <div className={`text-center py-12 border-2 border-dashed rounded-xl ${hasError("items") ? "border-red-300 bg-red-50" : "border-slate-200"}`}><div className={`w-12 h-12 ${hasError("items") ? "bg-red-100" : "bg-slate-100"} rounded-xl flex items-center justify-center mx-auto mb-3`}><ShoppingCart size={20} className={hasError("items") ? "text-red-400" : "text-slate-400"} /></div><p className={`text-sm font-medium ${hasError("items") ? "text-red-700" : "text-slate-700"}`}>No hay ítems</p><p className="text-xs text-slate-400 mt-1">{formData.invoiceType === "with-po" && selectedPO ? "Añade items de la PO o crea uno nuevo" : `Añade items a la ${currentDocType.label.toLowerCase()}`}</p></div>
                  : <div className="space-y-4">{items.map((item, index) => {
                    ...
                    return (<div key={item.id} className={`border rounded-xl p-5 ${itemComplete ? "border-emerald-200 bg-emerald-50/30" : "border-slate-200 bg-slate-50/50"}`}>
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">{itemComplete ? <CheckCircle2 size={12} className="text-emerald-600" /> : <Hash size={12} />}Item {index + 1}</span>
                      ...
                      <div className="space-y-4">
                        ...
                        {item.isNewItem ? <button onClick={() => { setCurrentItemIndex(index); setShowAccountModal(true); }} className={`w-full px-4 py-3 border ${hasError(`item_${index}_account`) ? "border-red-300 bg-red-50" : item.subAccountCode ? "border-emerald-200 bg-emerald-50" : "border-slate-200"} rounded-xl text-sm text-left flex items-center justify-between hover:border-slate-300 bg-white`}>{item.subAccountCode ? <div className="flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-600" /><span className="font-mono text-slate-900">{item.subAccountCode} - {item.subAccountDescription}</span></div> : <span className="text-slate-400">Seleccionar cuenta...</span>}<Search size={14} className="text-slate-400" /></button>
          ...
          <div className="lg:col-span-1">
            <div className="space-y-4">
              ...
              {/* Document Badge */}
              <div className={`${currentDocType.bgColor} border ${currentDocType.borderColor} rounded-2xl p-5`}><div className="flex items-center gap-3"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center"><DocIcon size={20} className={currentDocType.textColor} /></div><div><p className={`font-semibold ${currentDocType.textColor}`}>{currentDocType.label}</p><p className="text-sm text-slate-600">{getDocumentNumber()}</p></div></div></div>
              ...
              <div className={`border rounded-2xl overflow-hidden ${approvalPreview.autoApprove ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}>
                ...
                {!approvalPreview.autoApprove && approvalPreview.steps.length > 0 && <div className="px-5 py-4"><div className="space-y-3">{approvalPreview.steps.map((step, idx) => (<div key={step.id} className="flex items-start gap-3"><div className="flex flex-col items-center"><div className="w-7 h-7 rounded-full bg-amber-200 text-amber-800 flex items-center justify-center text-xs font-bold">{idx + 1}</div>{idx < approvalPreview.steps.length - 1 && <div className="w-0.5 h-8 bg-amber-200 mt-1" />}</div><div className="flex-1 pt-0.5"><p className="text-sm font-medium text-amber-900">{step.approverType === "role" && step.roles ? step.roles.join(", ") : step.approverType === "hod" ? "Jefe de departamento" : step.approverType === "coordinator" ? "Coordinador" : "Aprobador fijo"}</p>{step.approverNames.length > 0 && <div className="mt-1 flex flex-wrap gap-1">{step.approverNames.slice(0, 3).map((name, i) => <span key={i} className="inline-flex items-center gap-1 text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded-lg"><Users size={10} />{name.split(" ")[0]}</span>)}{step.approverNames.length > 3 && <span className="text-xs text-amber-700">+{step.approverNames.length - 3} más</span>}</div>}<p className="text-xs text-amber-700 mt-1">{step.requireAll ? "Todos deben aprobar" : `1 de ${step.approvers.length} debe aprobar`}</p></div></div>))}</div></div>}
              </div>

              {/* Info */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
                <div className="flex gap-3"><div className="w-8 h-8 bg-slate-200 rounded-lg flex items-center justify-center flex-shrink-0"><Info size={14} className="text-slate-500" /></div>
                  <div className="text-sm text-slate-600"><p className="font-medium text-slate-700 mb-2">Importante</p>
      ...
      {/* PO Modal */}
      {showPOModal && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden border border-slate-200">
        ...
        <div className="p-6"><div className="relative mb-4"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" value={poSearch} onChange={(e) => setPOSearch(e.target.value)} placeholder="Buscar por número o proveedor..." className="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 text-sm" autoFocus /></div>
          <div className="max-h-80 overflow-y-auto space-y-2">{filteredPOs.length === 0 ? <div className="text-center py-12"><div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-3"><FileText size={20} className="text-slate-400" /></div><p className="text-sm text-slate-500">No hay POs aprobadas</p></div> : filteredPOs.map((po) => <button key={po.id} onClick={() => selectPO(po)} className="w-full text-left p-4 border border-slate-200 rounded-xl hover:border-slate-300 hover:bg-slate-50"><div className="flex items-start justify-between"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><p className="font-semibold text-slate-900">PO-{po.number}</p><span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-lg font-medium">Aprobada</span></div><p className="text-sm text-slate-600">{po.supplier}</p><p className="text-xs text-slate-500 mt-1">{po.items.length} ítems</p></div><p className="font-semibold text-slate-900">{formatCurrency(po.totalAmount)} €</p></div></button>)}</div>
        </div>
      </div></div>}

      {/* PO Items Modal */}
      {showPOItemsModal && selectedPO && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[80vh] overflow-hidden border border-slate-200">
        ...
        <div className="p-6 max-h-96 overflow-y-auto"><div className="space-y-3">{poItemsWithInvoiced.map((poItem, idx) => { const isOver = poItem.availableAmount < 0; return <button key={poItem.id || idx} onClick={() => addPOItemToInvoice(poItem, idx)} className={`w-full text-left p-4 border rounded-xl ${isOver ? "border-red-200 bg-red-50 hover:border-red-300" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}><div className="flex items-start justify-between mb-2"><div className="flex-1"><p className="font-medium text-slate-900">{poItem.description || "Sin descripción"}</p><p className="text-xs text-slate-500 font-mono mt-1">{poItem.subAccountCode} - {poItem.subAccountDescription}</p></div><div className="text-right"><p className="font-semibold text-slate-900">{formatCurrency(poItem.totalAmount)} €</p><p className="text-xs text-slate-500">Total PO</p></div></div><div className="grid grid-cols-3 gap-4 text-sm mt-3 pt-3 border-t border-slate-100"><div><p className="text-slate-500 text-xs">Facturado</p><p className="font-medium text-slate-700">{formatCurrency(poItem.invoicedAmount)} €</p></div><div><p className="text-slate-500 text-xs">Disponible</p><p className={`font-medium ${isOver ? "text-red-600" : poItem.availableAmount < poItem.totalAmount * 0.1 ? "text-amber-600" : "text-emerald-600"}`}>{formatCurrency(poItem.availableAmount)} €</p></div><div className="text-right"><span className="inline-flex items-center gap-1 text-xs bg-slate-100 text-slate-700 px-2.5 py-1 rounded-lg"><Plus size={12} />Añadir</span></div></div>{isOver && <div className="flex items-center gap-2 mt-3 text-xs text-red-600"><AlertTriangle size={12} />Este item ya está sobre-facturado</div>}</button>; })}</div></div>
      </div></div>}

      {/* Supplier Modal */}
      {showSupplierModal && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden border border-slate-200">
        ...
        <div className="p-6"><div className="relative mb-4"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" value={supplierSearch} onChange={(e) => setSupplierSearch(e.target.value)} placeholder="Buscar por nombre o NIF..." className="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 text-sm" autoFocus /></div>
          <div className="max-h-80 overflow-y-auto space-y-2">{filteredSuppliers.length === 0 ? <div className="text-center py-12"><div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-3"><Building2 size={20} className="text-slate-400" /></div><p className="text-sm text-slate-500">No encontrado</p></div> : filteredSuppliers.map((s) => <button key={s.id} onClick={() => selectSupplier(s)} className="w-full text-left p-4 border border-slate-200 rounded-xl hover:border-slate-300 hover:bg-slate-50"><div className="flex items-start justify-between"><div className="flex-1"><p className="font-medium text-slate-900">{s.fiscalName}</p>{s.commercialName && <p className="text-sm text-slate-500">{s.commercialName}</p>}<p className="text-xs text-slate-500 mt-1 flex items-center gap-1"><Hash size={10} />{s.taxId}</p></div><Building2 size={16} className="text-slate-300" /></div></button>)}</div>
        </div>
      </div></div>}

      {/* Account Modal */}
      {showAccountModal && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[80vh] overflow-hidden border border-slate-200">
