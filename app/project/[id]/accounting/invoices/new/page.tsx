...
export default function NewInvoicePage() {
    ...
    try {
      ...
      for (const accDoc of accsSnap.docs) {
        ...
        const subsSnap = await getDocs(
          query(
            collection(db, `projects/${id}/accounts/${accDoc.id}/subaccounts`),
            orderBy("code", "asc")
          )
        );
  ...
  const updateAvailableWithCurrentItems = () => {
    ...
    items.forEach((item) => {
      if (item.poItemId) {
        currentInvoiceByItem[item.poItemId] =
          (currentInvoiceByItem[item.poItemId] || 0) + item.totalAmount;
      }
    });
    ...
    setPOItemsWithInvoiced((prev) =>
      prev.map((poItem) => {
        ...
        const baseInvoiced = poItem.invoicedAmount;
        return {
          ...poItem,
          availableAmount: poItem.totalAmount - baseInvoiced - currentAmount,
        };
      })
    );
  };
  ...
  const getApprovalPreview = () => {
    ...
    if (steps.every((s) => s.approvers.length === 0))
      return { autoApprove: true, message: "Irá a pendiente de pago" };
    return {
      autoApprove: false,
  ...
  const validateForm = () => {
    ...
    if (formData.invoiceType === "without-po" && !formData.supplier)
      e.supplier = "Selecciona un proveedor";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        invoiceData.autoApproved ? "Factura creada y pendiente de pago" : "Factura enviada para aprobación"
      );
  ...
  const formatCurrency = (a: number) =>
    new Intl.NumberFormat("es-ES", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(a);
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-slate-900">
                            {formatCurrency(poItem.totalAmount)} €
                          </p>
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm mt-3 pt-3 border-t border-slate-100">
                        <div>
                          ...
                          <p className="font-medium text-slate-700">
                            {formatCurrency(poItem.invoicedAmount)} €
                          </p>
                        </div>
                        <div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          {s.commercialName && (
                            <p className="text-sm text-slate-500">{s.commercialName}</p>
                          )}
